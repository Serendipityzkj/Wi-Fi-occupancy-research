...
CNN_MODEL_PATH = os.path.join("CNN-GBC model development", "CNN_model_output", "model")
GBC_MODEL_PATH = os.path.join("CNN-GBC model development", "gradient_boosting_model_best.pkl")
RF_MODEL_PATH = os.path.join("RF model development", "random_forest_model_best.pkl")
...
def load_test_data(file_path, required_cols):
    ...
    if not all(col in df.columns for col in required_cols):
        print("-ERROR: The test file is missing required RSSI columns.")
    ...
    for i in range(DEMO_STEPS):
        ...
        if len(state_history) < RF_PAST_STATES:
            ...
            if rf_prediction == 1:
                ...
                reason = "(RF CORRECTION: 'outside' changed to 'stay' based on history)"
            else:
                reason = "(RF validation: RF model agreed with 'outside')"
        else:
...
def main():
    ...
    print("      and fed to GBC for a final state prediction.")
    print("   2. 'CNN-GBC-RF': RF model corrects the 'outside' prediction")
